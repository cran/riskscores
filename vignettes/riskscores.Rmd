...
To view a dataframe with the full cross-validation results (including deviance, accuracy, AUC, and AUPRC metrics), run `cv_results$results`.
...
By default, `risk_mod()` fits with `method = "annealscore"`, which at each iteration proposes a candidate coefficient update by regressing the model's residuals on a random subset of covariates. The size of that subset is controlled by `p_subset_frac` (default `0.5`, i.e. half of the covariates are considered at each step); smaller values speed up each iteration at the cost of a narrower search, while larger values broaden the search at increased computational cost.
...
Predicted probabilities from `predict(mod, type = "response")` reflect the outcome prevalence in the training data. If the model will be applied to a population with a different, known outcome prevalence (e.g. a case-control sample fit to a general-population prevalence), pass a `target_prevalence` to rescale the predictions accordingly. This adjustment shifts predicted probabilities on the log-odds scale and leaves discrimination (AUC, AUPRC) unchanged while improving calibration.
...
By default, the sample's own outcome prevalence (`mean(y)`) is used as the reference; a different reference can be supplied via `study_prevalence`. The same logic underlies `prevalence_adjust()`, which can be applied directly to a vector of predicted probabilities.
